...
url = 'http://data.nba.net/json/cms/noseason/sportsmeta/nba_teams.json'
schedule_serialized = open(url).read
teamIds_json = JSON.parse(schedule_serialized)

teamIds_json['sports_content']['teams']['team'].each do |x|
  if x['is_nba_team'] == TRUE
    Team.create(
      teamId: x['team_id'],
      teamName: x['team_name'],
      teamNickname: x['team_nickname'],
      teamShortName: x['team_short_name']
      )
  end
end
...
game_info_array = []
  ...
  gameIds_json['league']['standard'].each do |x|
    if x['statusNum'] == 3 && x['seasonStageId'] == 2
      Game.create(
        gameId: x['gameId'],
        gameDateCode: x['startDateEastern'],
        HomeTeamId: x['hTeam']['teamId'],
        VisitorTeamId: x['vTeam']['teamId']
        )
...
url = "http://data.nba.net/json/cms/noseason/game/#{x[:gameDateCode]}/#{x[:gameIdCode]}/boxscore.json"
    ...
    boxscore_visitor = boxscore_json['sports_content']['game']['visitor']['players']['player']
